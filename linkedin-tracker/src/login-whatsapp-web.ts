...
// 5 minutes, matching login:linkedin. Three was tight enough that finding the
// phone and walking Linked Devices → Link a Device could run it out.
const LOGIN_TIMEOUT_MS = 300_000;

console.log('Opened WhatsApp Web. Scan the QR code with your phone now.');
console.log(`Waiting for login (up to ${LOGIN_TIMEOUT_MS / 60_000} minutes)... the QR showing is expected until you scan.`);
...
try {
  await chatList.first().waitFor({ state: 'visible', timeout: LOGIN_TIMEOUT_MS });
  console.log('✅ Logged in. Session saved to whatsapp-web-profile/. You can close this.');
  await ctx.close();
  process.exit(0);
} catch {
  console.error(`❌ Not logged in within ${LOGIN_TIMEOUT_MS / 60_000} minutes (QR not scanned). Re-run to try again.`);
  await ctx.close();
  process.exit(1);
}
